...
// Check slider status.
add_filter( 'mediclean_filter_slider_status', 'mediclean_check_slider_status' );

// Add slider to the theme.
add_action( 'mediclean_action_before_content', 'mediclean_add_featured_slider', 5 );

// Slider details.
add_filter( 'mediclean_filter_slider_details', 'mediclean_get_slider_details' );


if ( ! function_exists( 'mediclean_get_slider_details' ) ) :
	/**
	 * Slider details.
	 *
	 * @since 1.0.0
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							$slides[ $cnt ]['excerpt'] = mediclean_the_excerpt( apply_filters( 'mediclean_filter_slider_caption_length', 30 ), $post );
...
if ( ! function_exists( 'mediclean_add_featured_slider' ) ) :
	/**
	 * Add featured slider.
	 *
	 * @since 1.0.0
	 */
	function mediclean_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'mediclean_filter_slider_status', false );
		if ( true !== $flag_apply_slider ) {
			return false;
		}

		$slider_details = array();
		$slider_details = apply_filters( 'mediclean_filter_slider_details', $slider_details );
		...
		// Render slider now.
		mediclean_render_featured_slider( $slider_details );

	}
endif;


if ( ! function_exists( 'mediclean_render_featured_slider' ) ) :
	/**
	 ...
	 */
	function mediclean_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}
		$featured_slider_transition_effect   = mediclean_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = mediclean_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = mediclean_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = mediclean_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = mediclean_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = mediclean_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = mediclean_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = mediclean_get_option( 'featured_slider_transition_delay' );
...
if( ! function_exists( 'mediclean_check_slider_status' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function mediclean_check_slider_status( $input ) {

		// Slider status.
		$featured_slider_status = mediclean_get_option( 'featured_slider_status' );
